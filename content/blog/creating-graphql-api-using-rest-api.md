...
GraphQL is a fun service to consume on the client side. With its documentation, playground, and state management options such as [relay](https://relay.dev/) or [apollo](https://www.apollographql.com/), developer experience has been a blast for myself and co-workers. There is a common misconception around having REST or GraphQL API's but not both. You can have both! When I was learning I thought it was all in GraphQL or all in REST. To learn more about the difference between these services visit [Understanding GraphQL through REST](../understanding-graphql). Many of us are in a situation where we have an existing REST API that we don't want to completely give up on to start to try out GraphQL.
...
Project setup has some optional steps such as using [nodemon](https://nodemon.io/) to reload our app for us, and [dotenv](https://github.com/motdotla/dotenv) to load in environment variables. This would be where you would add in the tools you prefer to work with in JavaScript.
...
Inspect at http://localhost:9000 to see your schema defined. If you happen to try querying `hello` you will realized response is null. We need to still define our resolver response.
...
### Create .env file and add in our apiKey from the introduction
...
### Add data source to apollo-graphql server declaration in src/index.js
...
### Update resolvers to resolve correctly in src/resolvers.js
...
# Part 3. Schema first vs. Code First

If you're interested in learning the pro's and con's behind these two GraphQL approaches, stay tuned for my next post where we will take the API written in this tutorial and take it a step further by switching from schema first to code first and scaling up with better developer tooling. I'm kind of winging it on where to go next. So if you want to see something specifically let me know!
